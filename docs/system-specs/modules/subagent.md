...
| `_MAX_QUEUED` | 32 | Cap on the spawn queue depth for **non-batched** `spawn()` calls. Mirrors `_MAX_CONCURRENT`: the running side was already bounded; this constant closes the symmetric side so a sustained flood of non-batched spawns cannot grow the queue linearly with the spawn rate. **Batched calls (`batch_id` set) skip the cap entirely** — a wave's worst case is already bounded by `batch_total` (transport-clamped at 1000), and refusing a wave member mid-fan-out would strand the digest, because the spawn discipline forbids the parent from retrying mid-wave. The refusal mirrors the existing `refused_memory_critical` shape: a typed `refused_queue_full` SEL outcome, a `done=True` `SubagentInfo` with a retry-later error string, and no entry appended to `_queue`. |
