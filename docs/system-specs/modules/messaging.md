...
| `messaging/driver.py` | **Layer 2** — `TurnDriver` (channel-neutral turn loop), approval-mode constants, `resolve_transport_approval_mode` (the shared transport approval-mode resolver the nine direct-flag channel gateways delegate to), `_redact` helper |
...
**One resolver for the nine direct-flag channels.** `messaging.driver.resolve_transport_approval_mode(orch)` is the single implementation that Discord, Feishu, iMessage, Teams, Telegram, Webex, WeCom, Weixin and WhatsApp reach through their own named `_resolve_approval_mode(orch)` seam. It reads the CLI `--approval` override carried on `orch._approval_mode` first — the literal `"yolo"` is an unconditional grant — then falls back to the configured `agent.approval_mode`, and collapses anything that is not exactly `APPROVAL_AUTO` to `APPROVAL_INTERACTIVE`, so an unrecognized spelling is deny-by-default rather than an accidental auto-approve. The per-channel name stays, because channel tests import it: the shared helper is reached *through* that seam, not in place of it.

**Slack is the deliberate exception.** Its transport resolver reports the `--approval` flag as interactive and lets the gateway's own approval-event path apply the `yolo` and `reads` grants, because those classify the tool event rather than the turn; `slack/gateway.py::_interactive_approval` is where the `yolo` grant actually lands. Its seam therefore keeps its own body, and `test/test_slack_approval_resolve.py` pins that difference alongside the per-channel tests that pin the nine.
