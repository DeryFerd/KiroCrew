...
- `platform_compat.get_process_start_id()` is preferred — the same producer session
  ...
  `FILETIME` through the same query-only handle seams on Windows, read in
  creation-only mode so the event-loop caller never sits under the exit-time
  publication poll.
- `platform_compat.process_start_time()` is the fallback for a host where the
  preferred read fails, and it answers the same Windows token: both helpers read
  the process creation `FILETIME` (100-ns units) through the same query-only
  handle there, and on other POSIX it degrades to `ps -o lstart=`. `metrics.md`
