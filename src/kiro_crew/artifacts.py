...
class ArtifactStore:
    ...
    def _read_text(self, path: Path) -> str:
        ...
        * the resolved path must be confined to the store root, which closes the
        ...
        # The sensitive-path gate, first: `security.md` rule 1 requires every file
        # read to pass it, and it fences the credential leaves, which is a
        # different boundary than the store-root confinement below.
        if is_sensitive_path(str(resolved)):
        ...
        try:
            ...
            if fd >= 0:
                try:
                    ...
                    pass
        # ``UnicodeDecodeError`` propagates as it always has: it is a
        # ``ValueError``, and the handlers around this store catch that, so
        # converting it to ``ArtifactError`` would silently move it out of their
        # reach.
